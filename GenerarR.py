...
def reporte_experiencia(hojas_vida, anios_minimos):
    ...
    for hv in hojas_vida:
        años = calcular_experiencia_total(hv.get("experiencia", []))
        if años >= anios_minimos:
            resultado.append([hv["datos_personales"]["nombre"], años])
    ...
    with open(archivo, "w", encoding="utf-8") as f:
        if resumido:
            resumen = [{"Nombre": hv["Datos_personales"]["Nombre"],
                        "Correo Electronico": hv["Datos_personales"]["Correo Electronico"],
                        "Documento": hv["Datos_personales"]["Documento"]} for hv in hojas_vida]
            json.dump(resumen, f, indent=4, ensure_ascii=False)
        else:
            json.dump(hojas_vida, f, indent=4, ensure_ascii=False)

#Exportar archivo en formato csv
def exportar_csv(hojas_vida, archivo="Reportes.csv"):
    with open(archivo, mode="w", newline="", encoding="utf-8") as f:
        ...
        for hv in hojas_vida:
            ...
            años = calcular_experiencia_total(hv.get("experiencia_profesional", []))
            writer.writerow([nombre, correo, doc, años])

#Exportar archivo en formato txt
